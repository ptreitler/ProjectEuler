...
fun main() {
    ...
    var diagonalCount = 1
    var primeDiagonalCount = 0
    var primeRatio = 1.0
    while (primeRatio >= 0.1) {
        repeat(4) {
            currentDiagonal += (size + 1)
            if (currentDiagonal.isPrime()) primeDiagonalCount++
            diagonalCount++
        }

        primeRatio = primeDiagonalCount.toDouble() / diagonalCount.toDouble()

        size += 2
        println("size: $size, primes: $primeDiagonalCount/$diagonalCount, ratio: $primeRatio")
    }
